    ...
    <div className="space-y-2">
      ...
      {isBadge ? (
        ...
        <p className={cn("text-3xl font-headline font-bold tabular-nums", color)}>
          {value}{" "}
          <span className="text-[10px] font-black text-zinc-500 ml-1 uppercase">{unit}</span>
        </p>
      )}
    </div>
  );
...
export default function CalculatorPage() {
  ...
  const { data: profile } = useDoc<UserProfile>(
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Environmental telemetry updated successfully.",
      });
    } catch {
      // Firestore rules not yet deployed — local scores already updated above.
      toast({
        title: "Audit Saved Locally",
        description: "Score updated. Full sync will resume once database rules are deployed.",
      });
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4 space-y-12 animate-fade-in">
      <header className="text-center space-y-3">
        <h1 className="text-4xl font-headline font-bold text-foreground tracking-tight italic">
          Carbon Impact Audit
        </h1>
        <p className="text-zinc-600 text-sm max-w-md mx-auto font-medium">
          Verify your transportation footprint with real-time telemetry verification.
        </p>
      </header>
      ...
      <Card className="bg-white border-zinc-100 shadow-sm rounded-[2.5rem] overflow-hidden">
        <CardContent className="p-10 space-y-10">
          ...
          <div className="space-y-4">
            ...
            >
              <span id="transport-mode-label-sr" className="sr-only">
          ...
          >
            {calculating ? (
              <Spinner className="h-6 w-6 text-white" label="Executing impact audit..." />
            ) : (
              "Initiate Impact Audit"
            )}
          </Button>
        </CardContent>
      </Card>
          ...
          <Card className="bg-white border-zinc-200 shadow-sm rounded-[2.5rem] overflow-hidden">
            <div className="p-10 space-y-10">
              <div className="flex justify-between items-center">
                <h3 className="font-headline font-bold text-2xl tracking-tight italic">
                  Audit Verification
                </h3>
                <Badge
                  className={cn(
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-10 border-y border-black/5 py-10">
                <MetricDisplay
                  label="Total Dist"
                  value={activeResult.distance}
                  unit="KM"
                  color="text-foreground"
                />
                <MetricDisplay
                  label="CO₂ Generated"
                  value={activeResult.co2}
                  unit="KG"
                  color="text-red-500"
                />
                <MetricDisplay
                  label="Reward Credit"
                  value={activeResult.points}
                  unit="PTS"
                  color="text-primary"
                />
                <MetricDisplay
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  {saving ? (
                    <Spinner className="h-5 w-5" label="Committing telemetry..." />
                  ) : (
                    "Commit Telemetry"
                  )}
                </Button>
