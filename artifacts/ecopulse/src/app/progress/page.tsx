...
import { Milestone, Award, TrendingDown, Sparkles, Target } from "lucide-react";
import { cn } from "@/lib/utils";
import { buildUserCalculatorRecordsQuery } from "@/lib/firestore-queries";

const AreaChartComponent = lazy(() => import("@/components/charts/area-chart"));
...
export default function ProgressPage() {
  ...
  const { data: rawRecords, isLoading } = useCollection<CalculatorRecord>(
    recordsQuery as unknown as import("firebase/firestore").Query<CalculatorRecord> | null
  );
  ...
  const records = useMemo(() => {
    ...
    return [...rawRecords].sort((a, b) => {
      const aTs =
        typeof a.timestamp === "object" && a.timestamp && "toDate" in a.timestamp
          ? a.timestamp.toDate().getTime()
          : typeof a.timestamp === "number"
            ? a.timestamp
            : 0;
      const bTs =
        typeof b.timestamp === "object" && b.timestamp && "toDate" in b.timestamp
  ...
  const chartData = useMemo(() => {
    ...
    return records.map((r) => ({
      date: new Date(
        typeof r.timestamp === "object" && "toDate" in r.timestamp
          ? r.timestamp.toDate()
          : r.timestamp
    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-20 animate-fade-in">
      <header className="space-y-4">
        ...
        <p className="text-muted-foreground text-base max-w-xl">
          Deep-dive into your historical environmental telemetry and milestone evolution.
        </p>
      </header>
          ...
          <CardHeader className="p-12 border-b border-zinc-50 flex flex-row items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">
                Kilograms CO2e Output per Audit
              </p>
            </div>
            ...
            {!hasData ? (
              <div className="h-64 flex flex-col items-center justify-center text-center space-y-6 opacity-30 select-none">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm max-w-[240px] leading-relaxed text-zinc-500">
                    Execute at least one impact audit to initialize trend analysis visualization.
                  </p>
                </div>
              </div>
            ) : (
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.2em]">
        <span className={cn(completed ? "line-through text-zinc-500" : "text-zinc-600")}>
          {label}
        </span>
      ...
      <div className="h-2 w-full bg-zinc-100 rounded-full overflow-hidden shadow-inner">
        <div
          className={cn(
            "h-full transition-all duration-1500 ease-in-out",
            completed ? "bg-primary shadow-[0_0_15px_rgba(16,185,129,0.3)]" : "bg-primary/40"
          )}
