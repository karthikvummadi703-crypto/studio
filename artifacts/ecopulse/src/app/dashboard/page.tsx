...
const HeroMetric = memo(
  ({ label, value, subValue, color, isSmall, statusLabel }: HeroMetricProps) => (
...
const KPICard = memo(({ label, value, unit, icon: Icon, color }: KPICardProps) => (
  <div className="bg-white rounded-2xl p-6 flex items-center justify-between group transition-all border border-zinc-100 shadow-sm">
    ...
    <div
      className={cn("p-3 rounded-xl bg-black/5 transition-transform group-hover:scale-110", color)}
    >
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const totalSaved =
      records?.reduce(
        (acc: number, curr: CarbonRecord) => acc + (curr.co2 < 2 ? 2 - curr.co2 : 0),
        0
      ) || 0;
    ...
    const transportScore =
      avgCO2 === null ? 0 : Math.round(Math.max(0, Math.min(100, 100 - avgCO2 * 8)));
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
          <p className="text-primary/60 text-[10px] font-black uppercase tracking-[0.3em]">
            Strategy Node Alpha
          </p>
        ...
        <div className="flex gap-4">
          <Link href="/calculator">
            <Button
              size="sm"
              className="bg-primary text-primary-foreground hover:scale-105 transition-all rounded-xl px-6 py-5 font-bold tracking-widest text-[10px] uppercase shadow-lg shadow-primary/20"
            >
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-4 gap-12 items-center">
          <div className="lg:col-span-3 space-y-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              <HeroMetric
                ...
                color="text-primary"
                statusLabel={
                  stats.score >= 70 ? "good" : stats.score >= 40 ? "average" : "needs improvement"
                }
              />
              <HeroMetric
                label="Green Pts"
              ...
              <HeroMetric
                label="Latest Audit"
                value={stats.latestCO2.toFixed(1)}
                subValue="KG"
                color="text-zinc-500"
              />
            </div>
          ...
          <div className="hidden lg:flex flex-col items-center justify-center space-y-4 p-8 bg-zinc-50 rounded-[2.5rem] border border-zinc-100 shadow-sm">
            <div className="relative flex items-center justify-center">
              ...
              >
                <title id="score-ring-title">
                  Sustainability score: {stats.score.toFixed(0)} out of 100
                </title>
                <circle
                ...
                <circle
                  ...
                  strokeWidth="8"
                  strokeDasharray="351"
                  strokeDashoffset={351 * (1 - stats.score / 100)}
                  strokeLinecap="round"
                  stroke="currentColor"
                  fill="transparent"
                  r="56"
            ...
            <Badge
              variant="outline"
              className="bg-white border-primary/20 text-primary text-[9px] font-black uppercase px-4 py-1"
            >
              Operational
            </Badge>
          </div>
      ...
      <section aria-labelledby="factors-heading">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 id="factors-heading" className="text-xl font-headline font-bold text-foreground">
            Score Factors
          </h2>
          <Badge
            variant="outline"
          ...
          {[
            {
              ...
              detail: stats.hasRecords
                ? `Avg ${stats.latestCO2.toFixed(1)} kg CO₂/trip`
                : "Log a journey to score",
              color: "text-sky-600",
              bg: "bg-sky-50",
              bar: "bg-sky-500",
            },
            ...
            <div
              key={label}
              className="bg-white rounded-[1.5rem] p-6 border border-zinc-100 shadow-sm space-y-4"
            >
      ...
      {!stats.hasRecords ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {activeChallenge ? (
              <>
                <CardHeader className="p-0 mb-8">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <Badge className="bg-primary text-white text-[9px] font-black tracking-[0.2em] uppercase mb-2">
                        Priority Task
                      </Badge>
                ...
                <div className="mt-8 pt-6 border-t border-black/5">
                  <Button
                    variant="ghost"
                    className="w-full justify-between text-primary font-bold group hover:bg-primary/5 rounded-xl py-6"
                  >
                    Update Telemetry{" "}
              ...
              <div className="flex flex-col items-center justify-center h-full text-center space-y-4 py-12">
                ...
                <p className="text-muted-foreground text-sm font-medium">
                  Synchronizing new environment tasks shortly.
                </p>
              </div>
            )}
          </Card>
          ...
          <div className="space-y-6">
            <KPICard
              label="Est. Saved"
              value={stats.totalSaved.toFixed(1)}
              unit="KG"
              icon={TrendingDown}
            ...
            <Card className="bg-zinc-900 text-white rounded-[2rem] p-6 border-none shadow-2xl relative overflow-hidden group">
              <div className="relative z-10 space-y-3">
                ...
                <p className="text-xs font-medium leading-relaxed text-zinc-300">
                  Based on your last audit of{" "}
                  <span className="text-primary font-bold">{stats.latestCO2}kg</span>, switching
                  your next commute to a <span className="text-white font-bold">Bicycle</span> will
                  increase your score by <span className="text-emerald-400 font-bold">+8%</span>.
                </p>
              </div>
            </Card>
          </div>
      ...
      <div className="w-full md:w-1/2 p-12 flex flex-col items-center justify-center text-center space-y-8 bg-white relative z-10">
        ...
        <div className="space-y-3">
          <h2 className="text-3xl font-headline font-bold text-foreground tracking-tight">
            Environmental Node Initialized
          </h2>
          <p className="text-muted-foreground max-w-sm mx-auto text-sm leading-relaxed font-medium">
            Your sustainability telemetry is currently at zero. Complete an impact audit to unlock
            AI reduction strategies and start earning green points.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
          <Link href="/calculator">
            <Button
              size="lg"
              className="h-14 px-10 bg-primary text-primary-foreground font-bold rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 transition-all"
            >
              Start First Audit
            </Button>
          </Link>
