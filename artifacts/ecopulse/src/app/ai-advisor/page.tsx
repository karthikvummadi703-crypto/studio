...
const ChatMessage = memo(({ message, isUser }: { message: IChatMessage; isUser: boolean }) => (
  <div className={cn("flex gap-4 group animate-fade-in", isUser ? "flex-row-reverse" : "flex-row")}>
    <div
      className={cn(
...
export default function AIAdvisorPage() {
  ...
  const activeChat = useMemo(
    () => chats.find((c: AIConversation) => c.id === activeChatId),
    [chats, activeChatId]
  );
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col md:flex-row gap-6 animate-fade-in">
              ...
              chats.map((chat: AIConversation) => (
                ...
                >
                  ...
                  <div className="overflow-hidden">
                    ...
                    <p
                      className={cn(
                        "text-[9px] uppercase tracking-tighter mt-0.5 font-black",
                        activeChatId === chat.id ? "text-primary-foreground/80" : "text-zinc-500"
                      )}
                    >
      ...
      <Card className="flex-1 flex flex-col h-full overflow-hidden bg-white border border-zinc-100 shadow-sm rounded-[2rem]">
        <CardHeader className="p-6 border-b border-zinc-100 flex flex-row items-center justify-between shrink-0">
          ...
          <Badge
            variant="outline"
            className="border-primary/20 text-primary text-[9px] font-black uppercase tracking-widest px-3 py-1"
          >
            Gemini Flash
          </Badge>
        </CardHeader>
        ...
        <CardContent className="flex-1 p-0 flex flex-col overflow-hidden">
          <ScrollArea className="flex-1 p-6 md:p-10">
            {messages.length === 0 && !loading && !streamingText && !pendingAIMessage ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-8 py-20">
                <div
                  className="w-20 h-20 bg-primary/10 rounded-[2.5rem] flex items-center justify-center mx-auto"
                  aria-hidden="true"
                >
                  <Leaf className="h-10 w-10 text-primary animate-pulse" />
                </div>
                <div className="space-y-3">
                  <h3 className="text-2xl font-headline font-bold text-zinc-800">
                    Strategy Engine
                  </h3>
              ...
              <div
                ...
                role="log"
                aria-live="polite"
                aria-label="Advisor conversation history"
              >
                ...
                {streamingText && (
                  <div
                    className="flex gap-4 flex-row animate-fade-in"
                    role="status"
                    aria-live="polite"
                  >
                ...
                {loading && !streamingText && (
                  <div
                    ...
                    role="status"
                    aria-live="polite"
                    aria-label="AI Generating response"
                  >
                ...
                {error && (
                  <div
                    className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 border border-red-200 text-red-700 text-[11px] font-black uppercase tracking-widest"
                    role="alert"
                  >
                    ...
                    <Button
                      variant="ghost"
                      className="ml-auto h-7 text-[10px] uppercase font-black"
                      onClick={() => handleSend()}
                    >
                      Retry
                    </Button>
          ...
          <div className="p-6 md:p-8 border-t border-zinc-100">
            <div className="max-w-4xl mx-auto relative group">
              ...
              >
                {loading ? (
                  <Spinner className="h-5 w-5" label="Sending query..." />
                ) : (
